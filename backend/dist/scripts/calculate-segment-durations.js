...
async function main() {
    ...
    try {
        ...
        const newSegments = new Map(); // key route|from|to
        ...
        for (const routeId of routesToProcess) {
            ...
            for (const dir of ['there', 'back']) {
                ...
                if (timeAt24Sec > totalTimeSec) {
                    ...
                    console.log(`Корекція ${routeId} ${dir}: час при ${CORRELATION_SPEED_KMH} км/год ${Math.round(timeAt24Sec)} с > сума ${totalTimeSec} с → фактор ${factor.toFixed(3)}`);
                }
            }
        }
