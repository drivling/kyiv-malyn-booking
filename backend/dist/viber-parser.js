...
function isValidCalendarDate(day, monthIndex) {
...
function extractDate(text, messageDate) {
    ...
    // Не беремо збіг одразу після "о/в/виїзд" — це майже завжди час (о 14.10, о 18.15).
    // "на 16.08" / "на 09.07" — дата поїздки; час після «на» зазвичай з двокрапкою («на 9:00»).
    ...
    while ((dateMatch = dateRe.exec(text)) !== null) {
        ...
        if (/(?:^|[\s,;:(])(?:в|о|виїзд)\s*$/i.test(before)) {
            continue;
        }
    ...
    const monthNameMatch = text.match(new RegExp(`(\\d{1,2})\\s+(${monthNames})(?:\\s+(\\d{2,4}))?`, 'i'));
    if (monthNameMatch) {
...
function extractTime(text) {
    ...
    const fmtRange = (h1, m1, h2, m2) => `${fmt(h1, m1)}-${fmt(h2, m2)}`;
    ...
    const dotRangePrefixed = normalized.match(/(?:в|о|виїзд)\s+(\d{1,2})\.(\d{2})\s*-\s*(\d{1,2})\.(\d{2})/i);
    if (dotRangePrefixed) {
...
function extractSeats(text) {
    ...
    const wordMatch = text.match(/(?:^|[^\p{L}])(одне|один|одна|два|двоє|двох|три|трьох|чотири)\s+(?:вільн\p{L}*\s+)?(?:місц(?!яц)|пасажир|позаду|людин)/iu);
    if (wordMatch) {
        return wordMap[wordMatch[1].toLowerCase()] ?? null;
    }
    // "2 пасажира", "3 особи", "4 місця" — не брати цифру з дати DD.MM (lookbehind на крапку)
    const seatsMatch = text.match(/(?<![.\d])(\d{1,2})\s*(?:вільн\p{L}*\s+)?(?:пасажир|особ|місц(?!яц)[еeіяь]?|людин)/iu);
    if (seatsMatch) {
...
const WEEKDAY_PREFIX = /^(понеділок|вівторок|середа|четвер|п['ʼ]?ятниц|субот|неділ|пн|вт|ср|чт|пт|сб|нд)/i;
/**
...
function extractNotes(text) {
    ...
    const hasVilni = /(?:^|[^\p{L}])є\s+вільн\p{L}*\s+місц/iu.test(text) ||
        /(?:^|[^\p{L}])маю\s+(?:\d+\s+)?вільн\p{L}*\s+місц/iu.test(text);
    if (hasExactYeMitsya || hasVilni) {
        const upper = /(?:^|[^\p{L}])Є\s+(?:місця|вільн)/u.test(text) || /(?:^|[^\p{L}])Маю\s+вільн/u.test(text);
        push(upper ? 'Є місця' : 'є місця');
    }
    // 2. Розсадка «позаду» / «два місця позаду» (\b не працює з кирилицею)
    const rear = text.match(/(?:^|[^\p{L}])((?:двоє|два|одне|один|[123])\s+(?:місц\p{L}*\s+)?позаду)(?=[^\p{L}]|$)/iu) ||
        text.match(/(?:^|[^\p{L}])(позаду\s+(?:двоє|два|одне|один|\d+))(?=[^\p{L}]|$)/iu);
    if (rear) {
        const r = rear[1].toLowerCase().replace(/\s+/g, ' ').trim();
        if (/^(?:двоє|два|2)(?:\s+місц\p{L}*)?\s+позаду$/u.test(r) ||
            /^позаду\s+(?:двоє|два|2)$/.test(r)) {
            push('Двоє позаду');
        }
        else if (/^(?:одне|один|1)(?:\s+місц\p{L}*)?\s+позаду$/u.test(r) ||
    ...
    // 4. від + орієнтир
    const fromMatch = text.match(/від\s+((?:південного\s+)?(?:залізничного\s+)?вокзалу|м\.?\s*[\p{L}'ʼ-]{2,30}|ст\.?\s*м\.?\s*[\p{L}'ʼ.\s-]{2,30}|[\p{L}][\p{L}'ʼ-]{2,40})/iu);
    if (fromMatch) {
    ...
    if (/(?:^|[^\p{L}])до\s+цирку(?=[^\p{L}]|$)/iu.test(text)) {
        ...
        const toMatch = text.match(/(?:^|[^\p{L}])до\s+((?:південного\s+)?(?:залізничного\s+)?вокзалу|метро\s+[\p{L}.\s-]{2,25}|м\.?\s*[\p{L}'ʼ.-]{2,25})(?=[^\p{L}]|$)/iu);
        if (toMatch) {
    ...
    if (/академмістечко\s*[-–—]\s*малин|малин\s*[-–—]\s*академмістечко/i.test(text)) {
        ...
        const akadInKyiv = text.match(/ки[їєи][вї][а-я]*\s*\(\s*(Акад\.?|Академ\.?|Академмістечко)\s*\)/i);
        ...
        if (akadInKyiv) {
            ...
            push('Академмістечко');
            if (/теремки/i.test(akadLoose[1]))
                push('Теремки');
        }
    }
    const districtParen = text.match(/\(\s*(Шулявка|Оболонь|Почайна|Чайки|Лук['ʼ]?янівська(?:\s*\/\s*Академ)?|(?:південн\p{L}*\s+)?(?:залізничн\p{L}*\s+)?вокзал)\s*\)/iu);
    if (districtParen) {
    ...
    const stationInRoute = text.match(/(?:ки[їєи][вї][а-я]*|малин)\s*[–\-—(]\s*\(?\s*((?:південн\p{L}*\s+)?(?:залізничн\p{L}*\s+)?вокзал)\s*\)?/iu);
    ...
    if ((/\*ірпінь\s+малин\*/i.test(text) || /київ\s+\*?ірпінь(?=[^\p{L}]|$)/iu.test(text)) &&
        !parts.some((p) => /ірпінь/i.test(p))) {
        push('Ірпінь -> Малин');
    }
    // 10. Пес у авто
    if (/(?:добро.?зичлив\p{L}*\s+)?пес/iu.test(text) &&
    ...
    for (const m of text.matchAll(/\(([^)]{8,120})\)/g)) {
        ...
        if (/^\+?\d[\d\s()-]{7,}$/.test(inner))
            continue;
        if (WEEKDAY_PREFIX.test(inner) && /\d/.test(inner))
            continue;
        ...
        if (/дзвоніть|пишіть/i.test(inner) && parts.some((p) => /житомирськ/i.test(p)))
            continue;
        push(`(${inner})`);
    }
    if (parts.length === 0)
...
function extractListingType(text) {
    ...
    // "потрібно місце" / "потрібно одне місце" / "потрібно 1 місце"
    if (/(?:хтось\s+їде|шукаю|шукаємо|їде\s+хтось)/i.test(text) ||
        /потрібно\s+(?:\d+\s+|одне\s+|один\s+|два\s+|двоє\s+|три\s+)?місц/i.test(text)) {
        return 'passenger';
    }
...
function extractSenderName(text) {
    ...
    const nameMatch = text.match(new RegExp(`\\]\\s*${BIDI_OPEN}+([^\\u2066-\\u2069\\u202A-\\u202E]+?)${BIDI_CLOSE}+\\s*:`));
    if (nameMatch) {
...
function extractMessageDate(text) {
    ...
    const dateMatch = text.match(new RegExp(`\\[\\s*(?:[\\p{L}]+\\s*,\\s*)?(\\d{1,2})\\s+(${monthNames})\\s+(\\d{4})\\s+[рг]\\.`, 'iu'));
    if (dateMatch) {
        const day = parseInt(dateMatch[1], 10);
...
function extractMessageBody(text) {
    ...
    const bodyMatch = text.match(new RegExp(`\\]\\s*${BIDI_OPEN}+[^\\u2066-\\u2069\\u202A-\\u202E]+?${BIDI_CLOSE}+\\s*:\\s*([\\s\\S]+)`));
    if (bodyMatch) {
    ...
    if (DATE_HEADER_RE.test(text)) {
        ...
        const withoutName = withoutHeader.replace(new RegExp(`^${BIDI_OPEN}+[^\\u2066-\\u2069\\u202A-\\u202E\\n]{1,60}${BIDI_CLOSE}+\\s*:\\s*`), '');
