...
exports.parseHhMmLoose = parseHhMmLoose;
exports.mapDaysNoteToActiveWeekdays = mapDaysNoteToActiveWeekdays;
exports.normalizeStationName = normalizeStationName;
exports.stationNameMatches = stationNameMatches;
exports.parseEltrainTimetable = parseEltrainTimetable;
exports.fetchEltrainPage = fetchEltrainPage;
exports.findTrainByNumber = findTrainByNumber;
exports.resolveTrainForSchedule = resolveTrainForSchedule;
exports.pickStationTimes = pickStationTimes;
exports.durationMinutesBetween = durationMinutesBetween;
const HHMM = /^([0-1]?\d|2[0-3]):([0-5]\d)$/;
function parseHhMmLoose(value) {
    if (!value)
        return null;
...
function mapDaysNoteToActiveWeekdays(note) {
    const n = note.toLowerCase().replace(/\s+/g, ' ').trim();
    if (!n || n.includes('щоденно'))
        return [1, 2, 3, 4, 5, 6, 7];
    // «по сб., нд.» / «по сб. нд.» — only weekend
    if (/(?:^|[^а-яіїє])по\s+сб/.test(n) && n.includes('нд'))
        return [6, 7];
    if (n.includes('по сб') && !n.includes('нд'))
        return [6];
    if (n.includes('крім сб') && n.includes('нд'))
        return [1, 2, 3, 4, 5];
    if (n.includes('крім нд') || n.includes('крім нед'))
        return [1, 2, 3, 4, 5, 6];
    if (n.includes('по нд') || n.includes('по нед') || n === 'нд' || n.startsWith('нд.'))
        return [7];
...
function stationNameMatches(stationLabel, needle, opts) {
    ...
    if (needleIsSpecific || labelIsSpecific) {
        ...
        if (a.includes('борщаг') && b.includes('борщаг'))
            return true;
        if (a.includes('святошин') && b.includes('святошин'))
            return true;
        if (a.includes('київ-пас') && b.includes('київ-пас'))
            return true;
        if (a.includes('киев-пас') && b.includes('киев-пас'))
            return true;
 ...
 */
function parseEltrainTimetable(html) {
    ...
    const trainMeta = [];
    const dayNoteRe = /(щоденно|крім\s+сб\.\s*,\s*нд\.|крім\s+сб\.,\s*нд\.|крім\s+нд\.|крім\s+сб\.|по\s+сб\.\s*,\s*нд\.|по\s+сб\.,\s*нд\.|по\s+нд\.|по\s+сб\.)/i;
    for (const m of html.matchAll(/href="\.\?tid=\d+"[^>]*>\s*(\d{3,4})\s*,([\s\S]{0,280}?)<\/(?:font|a)>/gi)) {
    ...
    if (trainMeta.length < 3) {
        ...
        for (const row of rows) {
            if (!/\d{3,4}\s*,/.test(row))
                continue;
            if (!/(щоденно|крім|по\s+)/i.test(row))
                continue;
            ...
            if (rowMeta.length > bestMeta.length)
                bestMeta = rowMeta;
        }
    ...
    const stations = [];
    const stationRe = /<font[^>]*>\s*(\d+)\s*<\/font>&nbsp;<\/td>\s*<td[^>]*>\s*(?:<a[^>]*>\s*)?(?:<b>)?([^<]+?)(?:<\/b>)?(?:\s*<\/a>)?\s*<\/td>/gi;
    let sm;
    ...
    const timeRows = [];
    for (const row of rows) {
        const timeCells = [...row.matchAll(/<td[^>]*(?:width\s*=\s*46|class=q0)[^>]*>([\s\S]*?)<\/td>/gi)].map((m) => stripTags(m[1] ?? '').replace(/\u00a0/g, ' ').trim());
        if (timeCells.length < 4)
            continue;
        const onlyTimes = timeCells.every((t) => !t || t === '–' || t === '—' || t === '-' || HHMM.test(t));
        if (!onlyTimes)
            continue;
    ...
    for (let ti = 0; ti < trainCount; ti++) {
        ...
        if (!stops.some((s) => s.arrival || s.departure))
            continue;
        trains.push({
...
async function fetchEltrainPage(url) {
    const u = url.trim();
...
function findTrainByNumber(trains, tripNumber) {
 ...
 */
function resolveTrainForSchedule(trains, tripNumber, boardNeedle) {
    const found = findTrainByNumber(trains, tripNumber);
    if (found.length === 0)
        return { train: null, status: 'not_found' };
    if (found.length === 1)
        return { train: found[0], status: 'ok' };
    const needle = boardNeedle.trim();
    if (!needle)
        return { train: null, status: 'ambiguous' };
    const originOf = (t) => t.stops.find((s) => s.departure) ?? null;
    ...
    if (byExactOrigin.length === 1)
        return { train: byExactOrigin[0], status: 'ok' };
    const bySoftOrigin = found.filter((t) => {
        const origin = originOf(t);
        return origin && stationNameMatches(origin.name, needle);
    });
    if (bySoftOrigin.length === 1)
        return { train: bySoftOrigin[0], status: 'ok' };
    return { train: null, status: 'ambiguous' };
}
function pickStationTimes(train, boardNeedle, alightNeedle) {
    const exact = /святошин|борщаг|київ-пас|киев-пас|північна/i.test(boardNeedle);
    const boardStops = train.stops.filter((s) => stationNameMatches(s.name, boardNeedle, exact ? { exactKyivTerminals: true } : undefined));
...
function durationMinutesBetween(dep, arr) {
    if (!dep || !arr)
        return null;
