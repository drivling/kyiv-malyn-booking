...
exports.describeTelegramUserSessionError = describeTelegramUserSessionError;
...
let spawnChild = child_process_1.spawn;
/** Юніт-тести: підміна spawn (Python Telethon) без реальних процесів. */
function setSpawnForTests(fn) {
    spawnChild = fn ?? child_process_1.spawn;
}
function resetSpawnForTests() {
    spawnChild = child_process_1.spawn;
}
/** Черга: один Telethon-скрипт одночасно + пауза lunch.listener на час сесії. */
let telegramUserSessionTail = Promise.resolve();
async function withTelegramUserSessionExclusive(fn) {
    let release;
    ...
    try {
        await (0, lunch_listener_1.pauseLunchListenerForExclusiveSession)();
        try {
            return await fn();
        }
        finally {
            (0, lunch_listener_1.resumeLunchListenerAfterExclusiveSession)();
        }
    }
    finally {
        release();
...
const addTelegramAwaitingMap = new Map(); // chatId -> since
...
async function sendBehaviorPromoMessage(chatId, scenarioKey, context) {
    ...
    try {
        ...
        await (0, revoke_telegram_bot_1.handleTelegramBotBlockedFromOutboundSend)(tgPrisma, e, { chatId });
        throw e;
    }
}
/**
...
async function createDriverListingFromState(chatId, state, notes, senderName) {
    ...
    if (person.id) {
        ...
        if (fullPerson?.referredByPersonId) {
            await bot?.sendMessage(chatId, '🎁 <b>Акція «Приведи друга»</b>\n\n' +
                'Щоб ваш запрошувач отримав бонус за водія: запросіть пасажирів (/invite). ' +
                'Коли хоча б один з них підтвердить поїздку фото (/confirmride) — нарахується винагорода.', { parse_mode: 'HTML' });
        }
    }
}
...
function rangesOverlapWithTolerance(a, b, toleranceMinutes) {
...
function isApproximateTimeMatch(timeA, timeB) {
    const a = parseTimeRangeForMatch(timeA);
    const b = parseTimeRangeForMatch(timeB);
    if (!a || !b)
        return false;
...
function resolveMatchType(timeA, timeB) {
    if (isExactTimeMatch(timeA, timeB))
        return 'exact';
    if (isApproximateTimeMatch(timeA, timeB))
        return 'approximate';
    return 'same_day';
}
...
async function loadItineraryPointIds(tripRouteId) {
    ...
    if (!stops.length)
        return null;
    return (0, poputky_od_1.orderedPointIdsFromStops)(stops);
}
...
async function findMatchingPassengersForDriver(driverListing) {
    ...
    for (const p of passengers) {
        const routeMatchKind = (0, poputky_od_1.classifyPoputkyRouteMatch)({
            driver: {
...
async function sendMatchMessageToPerson(phone, messageHtml, botOptions) {
    ...
    if (chatId && bot) {
        ...
        if (!sent && botSendErr) {
            await (0, revoke_telegram_bot_1.handleTelegramBotBlockedFromOutboundSend)(tgPrisma, botSendErr, {
                chatId,
                normalizedPhone,
            });
        }
...
async function notifyPassengerAboutDriverPair(driverListing, passengerListing, matchType, routeMatchKind = 'exact') {
    ...
    if (row?.passengerNotifiedAt)
        return { kind: 'skipped' };
    const timeLabel = matchType === 'exact'
    ...
    const result = await sendMatchMessageToPerson(passengerListing.phone, msg, matchType === 'same_day'
...
async function notifyDriverAboutPassengerPair(driverListing, passengerListing, matchType, routeMatchKind = 'exact') {
    const row = await tgPrisma.viberMatchPairNotification.findUnique({
        where: {
            passengerListingId_driverListingId: {
                passengerListingId: passengerListing.id,
    ...
    const result = await sendMatchMessageToPerson(driverListing.phone, msg, matchType === 'same_day'
        ? { ...(replyMarkup ? { replyMarkup } : {}), forceBotOnly: true }
        : {
            ...(replyMarkup ? { replyMarkup } : {}),
            matchType,
...
async function sendAdminNewListingMatchReport(listingId, listingType, pairCount, stats) {
    if (pairCount === 0 || !bot || !adminChatId)
        return;
    ...
    await bot
        .sendMessage(adminChatId, `🔔 <b>Збіги після збереження оголошення</b> #${listingId} (${typeUa})\n\n` +
...
async function notifyMatchingPassengersForNewDriver(driverListing, driverChatId) {
    ...
    if (driverChatId && exactList.length > 0) {
        const lines = exactList.map((p) => {
            ...
            return `• 👤 ${(0, person_name_1.displayName)(p.senderName, 'Пасажир')} — ${time}\n  📞 ${formatPhoneTelLink(p.phone)}${p.notes ? `\n  📝 ${p.notes}` : ''}`;
        }).join('\n');
...
async function notifyMatchingDriversForNewPassenger(passengerListing, passengerChatId) {
    ...
    if (passengerChatId && exactList.length > 0) {
        const lines = exactList.map((d) => {
            ...
            return `• 🚗 ${(0, person_name_1.displayName)(d.senderName, 'Водій')} — ${time}, ${d.seats != null ? d.seats + ' місць' : '—'}\n  📞 ${formatPhoneTelLink(d.phone)}${d.notes ? `\n  📝 ${d.notes}` : ''}`;
        }).join('\n');
...
const getPersonByTelegramUsername = async (username) => {
    const normalized = (0, telegram_contact_1.normalizeTelegramUsername)(username);
    if (!normalized)
        return null;
...
exports.getPersonByTelegramUsername = getPersonByTelegramUsername;
/** Наступний технічний номер 380010000000, 380010000001, … для username-only контактів. */
async function getNextTechnicalPhoneNumber() {
    ...
    if (!latest)
        return telegram_contact_1.TECHNICAL_PHONE_PREFIX;
 ...
 */
const findOrCreatePersonByTelegramUsername = async (username, options) => {
    const normalizedUsername = (0, telegram_contact_1.normalizeTelegramUsername)(username);
    if (!normalizedUsername) {
        throw new Error('Empty telegram username');
    }
    const fullName = options?.fullName != null && String(options.fullName).trim() !== '' ? String(options.fullName).trim() : null;
    const existing = await (0, exports.getPersonByTelegramUsername)(normalizedUsername);
    if (existing) {
        const updates = {};
        if (fullName && !existing.fullName?.trim())
            updates.fullName = fullName;
        if (!existing.telegramUsername?.trim())
            updates.telegramUsername = normalizedUsername;
...
exports.findOrCreatePersonByTelegramUsername = findOrCreatePersonByTelegramUsername;
...
function formatPhoneTelLink(phone) {
    ...
    if (!p)
        return '—';
    if ((0, telegram_contact_1.isTelegramUsernameContact)(p)) {
        return (0, telegram_contact_1.formatTelegramContactHtmlLink)(p);
    }
    if ((0, telegram_contact_1.isTechnicalPlaceholderPhone)(p))
        return '—';
 ...
 */
const getRouteName = (route) => (0, poputky_od_1.formatOdRouteLabel)(route, poputky_od_1.DEFAULT_POINT_LABELS_UK);
exports.getTelegramRouteName = getRouteName;
async function loadPoputkyPoints() {
...
function buildPoputkyFromKeyboard(prefix, points) {
...
function buildPoputkyToKeyboard(prefix, points, fromCode) {
    const rows = points
...
const BOOK_DIRECTION_OPTIONS_FALLBACK = [
...
async function loadBookDirectionOptions() {
    try {
        const pairs = await (0, poputky_od_1.listOdPairs)(tgPrisma);
        if (pairs.length) {
            return pairs.map((p) => ({
...
async function buildBookDirectionKeyboard(prefix, emoji) {
...
function buildBookTypeKeyboard() {
    return {
        inline_keyboard: [
...
async function loadMarshrutkaOdPoints() {
    return tgPrisma.tripPoint.findMany({
...
function buildBookOdFromKeyboard(points) {
...
function buildBookOdToKeyboard(points, fromCode) {
    const rows = points
        .filter((p) => p.code !== fromCode)
...
function odPairLabel(fromName, toName) {
    return `${fromName} → ${toName}`;
}
function parseBookOdDateCallback(data, prefix) {
    ...
    if (!fromCode || !toCode || !/^\d{4}-\d{2}-\d{2}$/.test(dateYmd))
        return null;
...
function passengerRideHasOdAndDate(state) {
...
function buildElektrichkaPurchaseMessage(schedule, options) {
...
function buildElektrichkaPurchaseKeyboard(schedule) {
    const rows = [];
 ...
 */
async function markViberListingAuthorNotified(listingId) {
...
const sendViberListingConfirmationToUser = async (phone, listing) => {
    ...
    try {
        ...
        if (notifyState?.authorNotifiedAt) {
            console.log(`ℹ️ Viber оголошення #${listing.id}: автора вже сповіщено (${notifyState.authorNotifiedAt.toISOString()}), пропускаємо`);
            return;
        }
        if ((0, telegram_contact_1.isTelegramUsernameContact)(trimmed)) {
            const person = await (0, exports.getPersonByTelegramUsername)(trimmed);
            ...
            if (person && isTelegramUserSenderEnabled()) {
                ...
                sent = await sendMessageViaUserAccount(person.phoneNormalized, promoMessage, {
                    telegramUsername: (0, telegram_contact_1.normalizeTelegramUsername)(trimmed),
                });
                if (sent) {
                    await tgPrisma.person.update({
                        where: { id: person.id },
                    ...
                    console.log(`✅ Telegram: автору Viber оголошення #${listing.id} надіслано підтвердження по @${(0, telegram_contact_1.normalizeTelegramUsername)(trimmed)}`);
                }
            }
            if (!sent && person?.phoneNormalized && !(0, telegram_contact_1.isTechnicalPlaceholderPhone)(person.phoneNormalized)) {
                const r = await (0, sms_fallback_1.sendPaidFallbackSms)(tgPrisma, {
        ...
        if (!chatId || botSendFailedChatNotFound) {
            ...
            if (!sent) {
                ...
                if (r.sent) {
                    ...
                    console.log(`ℹ️ Viber оголошення #${listing.id}: автор недосяжний (Telegram + SMS), причина SMS: ${r.reason ?? 'n/a'}`);
                }
            }
        }
    }
 ...
 */
async function resolveUsernameByPhoneFromTelegram(phone) {
    const sessionPath = process.env.TELEGRAM_USER_SESSION_PATH?.trim();
    const scriptDir = sessionPath ? path_1.default.dirname(sessionPath) : '';
    const scriptPath = path_1.default.join(scriptDir, 'send_message.py');
    const apiId = process.env.TELEGRAM_API_ID;
 ...
 */
async function fetchTelegramGroupMessages(options) {
    const sessionPath = process.env.TELEGRAM_USER_SESSION_PATH?.trim();
    const scriptDir = sessionPath ? path_1.default.dirname(sessionPath) : '';
    const scriptPath = path_1.default.join(scriptDir, 'fetch_telegram_messages.py');
    const apiId = process.env.TELEGRAM_API_ID;
    const apiHash = process.env.TELEGRAM_API_HASH;
    ...
    return withTelegramUserSessionExclusive(async () => {
        ...
        if (newLastIdsJson) {
            try {
                const parsed = JSON.parse(newLastIdsJson);
                ...
                if (parsed && typeof parsed === 'object') {
                    for (const [k, v] of Object.entries(parsed)) {
                        if (v && typeof v === 'object') {
                            ...
                            (nested.PoDoroguem ?? (nested.PoDoroguem = {}))[k] = Number(v);
                        }
                    }
                }
                for (const [chat, topics] of Object.entries(nested)) {
                    for (const [topicStr, msgIdRaw] of Object.entries(topics)) {
                        ...
                        if (Number.isNaN(topicId) || !Number.isFinite(msgId) || msgId <= 0)
                            continue;
...
async function getAuthorChatIdForListing(listing) {
    if (listing.personId) {
    ...
    if (phone && !(0, telegram_contact_1.isTelegramUsernameContact)(phone)) {
        return (0, exports.getChatIdByPhone)(phone);
    }
    return null;
}
 ...
 */
function describeTelegramUserSessionError(detail) {
    const text = (detail || '').toLowerCase();
    if (!text)
        return null;
    if (text.includes('auth_key_duplicated') || text.includes('two different ip addresses')) {
        return ('🔑 Сесію Telegram-акаунта відкликано Telegram: один і той самий session-файл ' +
    ...
    if (text.includes('auth_key_unregistered') ||
        ...
        return ('🔑 Сесія Telegram-акаунта недійсна (відкликана або розлогінена). ' +
            'Перезапустіть авторизацію: backend/telegram-user → python3 auth_session.py.');
    }
    if (text.includes('database is locked')) {
        return ('⏳ Session-файл зайнятий іншим Telethon-процесом (lunch listener). ' +
    ...
    return new Promise((resolve) => {
        ...
        child.on('close', async (code) => {
                ...
                if (!isUsername && stdout.trim()) {
                    try {
                        ...
                        if (parsed.phone && parsed.telegramUsername?.trim()) {
                            await (0, exports.findOrCreatePersonByPhone)(parsed.phone, { telegramUsername: parsed.telegramUsername.trim() }).catch((e) => console.error('❌ Оновлення Person після send_message:', e));
                        }
                    }
                    catch {
            ...
            if (code === 1 && !isUsername) {
                ...
                recordTelegramUserSendError(value, isUsername ? 'username' : 'phone', code ?? 2, errorText).catch((e) => console.error('❌ recordTelegramUserSendError:', e));
            }
            resolve(false);
        });
        child.on('error', (err) => {
...
const sendRideShareRequestToDriver = async (requestId, driver, passenger) => {
    ...
    try {
        ...
        await (0, revoke_telegram_bot_1.handleTelegramBotBlockedFromOutboundSend)(tgPrisma, e, {
            chatId: driverChatId,
            normalizedPhone: (0, exports.normalizePhone)(driver.phone),
        });
...
<i>Бажаємо приємної подорожі! 🚐</i>
    `.trim();
        ...
        await (0, revoke_telegram_bot_1.handleTelegramBotBlockedFromOutboundSend)(tgPrisma, error, {
            chatId,
            personId: booking.personId ?? null,
            normalizedPhone: booking.phone ? (0, exports.normalizePhone)(booking.phone) : null,
        });
...
async function registerUserPhone(chatId, userId, phoneInput, telegramName) {
    ...
    try {
        ...
        if (totalBookings === 0) {
            ...
            const created = await (0, exports.findOrCreatePersonByPhone)(phoneInput, {
...
async function runAdminReferralReport(chatId) {
    if (!bot || !isTelegramAdminChat(chatId))
        return;
    try {
        const report = await (0, referral_1.buildAdminReferralSummary)(tgPrisma);
...
async function runAdminCheckClients(chatId) {
    ...
    const passengersByDate = new Map();
    for (const p of passengers) {
    ...
    for (const d of drivers) {
        ...
        for (const p of ps) {
            ...
            if (pairLinesByType[matchType].length < CHECKCLIENTS_PAIR_LINES_LIMIT) {
                ...
                pairLinesByType[matchType].push(`• #D${d.id}/#P${p.id} · ${getRouteName(d.route)} ↔ ${getRouteName(p.route)}${alongTag} · ${formatDate(d.date)} · 🚗 ${driverTime} ↔ 👤 ${passengerTime} (${timeDelta})`);
            }
            if (matchType === 'exact')
                exactPairCount++;
...
function setupBotCommands() {
    ...
    const startPassengerRideFromBookOd = async (chatId, userId, fromCode, toCode, dateYmd, messageId) => {
        ...
        const route = (0, poputky_od_1.buildOdRouteSlug)(from.code, to.code);
        const userPhone = await (0, exports.getPhoneByTelegramUser)(userId, chatId);
        const summary = '👤 <b>Шукаю попутку (пасажир)</b>\n\n' +
            `✅ Звідки: ${from.nameUk}\n` +
            `✅ Куди: ${to.nameUk}\n` +
    ...
    bot.onText(/\/start/, async (msg) => {
        ...
        if (rawStart.toLowerCase().startsWith('ref_')) {
            const ok = await (0, telegram_referral_1.handleReferralStartParam)(tgPrisma, chatId, rawStart);
            if (ok) {
        ...
        if (draftMatch) {
            ...
            if (draft && draft.role === role) {
                ...
                if (userPhone) {
                    if (role === 'driver') {
                        const state = { state: 'driver_ride_flow', step: 'notes', route: draft.route, fromPointId: draft.fromPointId ?? null, toPointId: draft.toPointId ?? null, date: draft.date, departureTime: draft.departureTime ?? undefined, seats: null, priceUah: draft.priceUah ?? null, phone: userPhone, since: Date.now() };
                        await createDriverListingFromState(chatId, state, draft.notes ?? null, senderName);
                    }
                    else {
                        const state = { state: 'passenger_ride_flow', step: 'notes', route: draft.route, fromPointId: draft.fromPointId ?? null, toPointId: draft.toPointId ?? null, date: draft.date, departureTime: draft.departureTime ?? null, phone: userPhone, since: Date.now() };
...
🌐 Забронювати квиток на сайті: https://malin.kiev.ua
    ...
    const handleBook = async (chatId, userId) => {
        ...
        await bot?.sendMessage(chatId, '🎫 <b>Нове бронювання</b>\n\nОберіть тип поїздки:', { parse_mode: 'HTML', reply_markup: buildBookTypeKeyboard() });
    };
...
Або кнопка «🛠 Адмін» у головному меню.`
    ...
    const runMenuCommand = async (chatId, userId, command) => {
        ...
        switch (command) {
            ...
            case '/invite': {
                ...
                if (!person || !bot)
                    return;
                return (0, telegram_referral_1.sendInviteProgramMessage)(bot, tgPrisma, chatId, person.id, telegramBotUsername);
            }
            case '/confirmride': {
                const person = await (0, exports.getPersonByTelegram)(userId, chatId);
                if (!person || !bot)
                    return;
                return (0, telegram_referral_1.startRideProofFlow)(bot, tgPrisma, chatId, person.id);
            }
            default:
                return;
        }
    ...
    bot.on('contact', async (msg) => {
        ...
        if (passengerState?.state === 'passenger_ride_flow' && passengerState.step === 'phone') {
            ...
            if (passengerRideHasOdAndDate({ ...passengerState, phone })) {
                ...
                await bot?.sendMessage(chatId, `🛣 ${getRouteName(passengerState.route)}\n📅 ${formatDate(new Date(passengerState.date))}\n\n🕐 Оберіть час (або Пропустити):`, { parse_mode: 'HTML', reply_markup: buildPassengerTimeKeyboard() });
                return;
            }
    ...
    const acceptRideProofPhoto = async (chatId, userId, fileId) => {
        if (!bot)
            return;
        const person = await (0, exports.getPersonByTelegram)(userId, chatId);
        if (!person)
            return;
        await (0, telegram_referral_1.handleRideProofPhoto)(bot, tgPrisma, chatId, person.id, fileId, (text, photoFileIds) => {
            if (!bot || !adminChatId)
                return;
            bot.sendMessage(adminChatId, text, { parse_mode: 'HTML' }).catch(() => { });
    ...
    bot.on('document', async (msg) => {
        ...
        if (!doc || !bot)
            return;
        const isImage = doc.mime_type?.startsWith('image/') || /\.(jpe?g|png|heic|webp)$/i.test(doc.file_name ?? '');
        if (!isImage)
            return;
    ...
    bot.on('message', async (msg) => {
        ...
        if (bot) {
            const remindedAboutPhoto = await (0, telegram_referral_1.remindRideProofExpectsPhoto)(bot, chatId).catch(() => false);
            if (remindedAboutPhoto)
                return;
        }
        if (!text)
            return;
        ...
        {
            const personForInvite = await (0, exports.getPersonByTelegram)(userId, chatId);
            if (personForInvite && bot) {
                const handledInvite = await (0, telegram_referral_1.handleReferralContactInput)(bot, tgPrisma, chatId, personForInvite.id, text).catch(() => false);
                if (handledInvite)
                    return;
            }
        }
        ...
        if (isTelegramAdminChat(chatId) && addTelegramAwaitingMap.has(chatId)) {
            ...
            try {
                const parsedMessages = (0, telegram_parser_1.parseTelegramMessages)(text);
                if (parsedMessages.length === 0) {
    ...
    bot.onText(/^\/confirmride(?:@\w+)?$/i, async (msg) => {
        ...
        await (0, telegram_referral_1.startRideProofFlow)(bot, tgPrisma, chatId, person.id);
    });
        ...
        try {
            ...
            if (data === 'addtelegram_fetch' || data === 'addtelegram_fetch_full') {
                ...
                if (rawText === null) {
                    ...
                    await bot?.editMessageText((sessionHint ??
                        ...
                        detail, { chat_id: chatId, message_id: statusMsg?.message_id, parse_mode: 'HTML' });
                    return;
                }
                if (!rawText.trim()) {
                    await bot?.editMessageText('✅ Немає нових повідомлень для імпорту. Усі вже оброблені.', { chat_id: chatId, message_id: statusMsg?.message_id });
                    return;
                }
                const parsedMessages = (0, telegram_parser_1.parseTelegramMessages)(rawText);
                if (parsedMessages.length === 0) {
                    await bot?.editMessageText('❌ Не вдалося розпарсити жодне повідомлення. Спробуйте /addtelegram → Вставити текст вручну.', { chat_id: chatId, message_id: statusMsg?.message_id });
                    return;
                }
                let created = 0;
                for (let i = 0; i < parsedMessages.length; i++) {
                ...
                await bot?.editMessageText(`✅ Завантажено з групи: створено ${created} оголошень з ${parsedMessages.length}.`, { chat_id: chatId, message_id: statusMsg?.message_id });
                return;
            }
            if (data === 'addtelegram_paste') {
                addTelegramAwaitingMap.set(chatId, Date.now());
                await bot?.sendMessage(chatId, '📋 Надішліть текст оголошення (переслати або вставити). Через 10 хв очікування скасується.', { parse_mode: 'HTML' });
            ...
            if (data.startsWith('adddriver_from_')) {
                ...
                await bot?.editMessageText(`📍 Звідки: ${fromPoint.nameUk}\n\n2️⃣ Куди їдете?`, { chat_id: chatId, message_id: messageId, parse_mode: 'HTML', reply_markup: toKeyboard });
                await bot?.answerCallbackQuery(query.id);
                return;
            }
            if (data.startsWith('adddriver_to_')) {
                ...
                const route = (0, poputky_od_1.buildOdRouteSlug)(fromPoint.code, toPoint.code);
                driverRideStateMap.set(chatId, {
                    ...state,
                    step: 'date',
                    route,
                ...
                await bot?.editMessageText(`🛣 Напрямок: ${getRouteName(route)}\n\n3️⃣ Оберіть дату:`, { chat_id: chatId, message_id: messageId, parse_mode: 'HTML', reply_markup: dateKeyboard });
                await bot?.answerCallbackQuery(query.id);
                return;
            }
            ...
            if (data === 'book_type_marshrutka') {
                ...
                if (points.length < 2) {
                    await bot?.editMessageText('❌ Немає доступних міст для бронювання маршрутки.\n\nАдмін має увімкнути «У звідки/куди» для міст.', { chat_id: chatId, message_id: messageId, parse_mode: 'HTML' });
                    await bot?.answerCallbackQuery(query.id);
                    return;
                }
            ...
            if (data === 'book_type_elektrichka') {
                await bot?.editMessageText('🎫 <b>Нове бронювання</b> · 🚆 Електричка\n\n1️⃣ Оберіть напрямок:', {
                    chat_id: chatId,
                    message_id: messageId,
                    parse_mode: 'HTML',
            ...
            if (data.startsWith('book_m_date_')) {
                ...
                const schedules = allSchedules.filter((s) => (0, schedule_trip_1.isScheduleActiveOnDate)(s.activeWeekdays, selectedDate) &&
                    (0, schedule_trip_1.scheduleMatchesOdAlongStops)(s.tripRoute?.stops, from.id, to.id));
                if (schedules.length === 0) {
                    await bot?.editMessageText('❌ <b>Немає доступних рейсів</b> за розкладом для цього напрямку.\n\n' +
                        `Напрямок: ${odPairLabel(from.nameUk, to.nameUk)}\n` +
                ...
                const timeButtons = await Promise.all(schedules.map(async (schedule) => {
                    ...
                    const boardTime = (0, schedule_trip_1.boardingTimeAtStop)(schedule.departureTime, schedule.tripRoute?.stops, from.id);
                    return {
            ...
            if (data.startsWith('book_m_poputky_')) {
                ...
                const ok = await startPassengerRideFromBookOd(chatId, userId, parsed.fromCode, parsed.toCode, parsed.dateYmd, messageId);
                if (!ok) {
                    await bot?.answerCallbackQuery(query.id, {
            ...
            if (data.startsWith('book_date_') && !data.startsWith('book_m_date_')) {
                await bot?.editMessageText('🎫 Цей крок застарів. Почніть бронювання знову: /book', {
                    chat_id: chatId,
                    message_id: messageId,
                    parse_mode: 'HTML',
                    reply_markup: buildBookTypeKeyboard(),
            ...
            if (data.startsWith('el_date_')) {
                ...
                const schedules = allSchedules.filter((s) => (0, schedule_trip_1.isScheduleActiveOnDate)(s.activeWeekdays, selectedDate));
                if (schedules.length === 0) {
                    await bot?.editMessageText('❌ <b>Немає рейсів електрички</b> на цю дату.\n\n' +
            ...
            if (data.startsWith('el_time_')) {
                ...
                if (!(0, schedule_trip_1.isScheduleActiveOnDate)(schedule.activeWeekdays, selectedDate)) {
                    await bot?.answerCallbackQuery(query.id, {
                ...
                const dateForCallback = selectedDate.replace(/-/g, '_');
                const direction = schedule.route.split('-').slice(0, 2).join('-') ||
            ...
            if (data.startsWith('book_sched_') &&
            ...
            if (data.startsWith('book_sched_seats_')) {
                ...
                const routeLabel = schedule?.tripRoute?.labelUk?.trim() || (schedule ? getRouteName(schedule.route) : '—');
                const confirmKeyboard = {
                    inline_keyboard: [
                        [
                            {
...
const productionTelegramBot = token ? new node_telegram_bot_api_1.default(token, { polling: true }) : null;
...
function startPendingReferralCodeCleanup() {
    const ONE_DAY_MS = 24 * 60 * 60 * 1000;
    const run = () => {
        (0, telegram_referral_1.cleanupExpiredPendingReferralCodes)(tgPrisma)
            .then((count) => {
            if (count > 0)
                console.log(`🧹 Видалено протермінованих ref-кодів: ${count}`);
        })
...
function setTelegramBotForTests(instance) {
    bot = instance;
}
function resetTelegramBotForTests() {
    bot = productionTelegramBot;
}
/**
