...
test('getChatIdByPhone: fallback на Booking', async () => {
  const findUnique = vi.fn(async () => ({ telegramChatId: null, phoneNormalized: '380501112233' }));
...
test('getPersonByPhone: findUnique з нормалізованим номером', async () => {
  const findUnique = vi.fn(async () => ({ id: 1, phoneNormalized: '380671234567' }));
  setTelegramPrismaForTests(asPrisma({ person: { findUnique } }));
  const p = await getPersonByPhone('0671234567');
  assert(p);
  assert.equal(p.id, 1);
  const firstCall = findUnique.mock.calls[0]?.[0] as
    | { where: { phoneNormalized: string } }
    | undefined;
  assert.equal(firstCall?.where?.phoneNormalized, '380671234567');
});
...
test('getNameByPhone: Person.fullName', async () => {
  setTelegramPrismaForTests(
    asPrisma({
      person: {
        findUnique: vi.fn(async () => ({ fullName: '  Іван  ' })),
      },
      booking: { findMany: vi.fn(async () => []) },
    })
  );
...
test('getNameByPhone: з Booking якщо Person без імені', async () => {
  setTelegramPrismaForTests(
    asPrisma({
      person: { findUnique: vi.fn(async () => ({ fullName: null })) },
      booking: {
        findMany: vi.fn(async () => [
...
test('getPhoneByTelegramUser: з Person', async () => {
  setTelegramPrismaForTests(
    asPrisma({
      person: {
        findFirst: vi.fn(async () => ({ phoneNormalized: '380501112233' })),
      },
...
test('getPhoneByTelegramUser: з Booking', async () => {
  setTelegramPrismaForTests(
    asPrisma({
      ...
      booking: {
        findFirst: vi.fn(async () => ({ phone: '0670000000' })),
      },
    })
  );
...
test('getDriverFutureBookingsForMybookings: з оголошеннями та бронюваннями', async () => {
  ...
  setTelegramPrismaForTests(
    asPrisma({
      person: { findFirst: vi.fn(async () => ({ id: 5 })) },
      viberListing: {
...
test('createOrMergeViberListing: merge за телефоном — update', async () => {
  ...
  const upCall = update.mock.calls[0]?.[0] as
    | { data: { rawMessage?: string } }
    | undefined;
  const rawMerged = String(upCall?.data?.rawMessage ?? '');
...
test('notifyMatchingPassengersForNewDriver: findMany для пасажирів і завершення без throw', async () => {
  ...
  const findMany = vi.fn(async () => [passenger]);
  const findUnique = vi.fn(async () => null);
  const upsert = vi.fn(async () => ({}));
  setTelegramPrismaForTests(
    asPrisma({
...
test('notifyPassengerAboutDriverPair: skipped якщо вже passengerNotifiedAt', async () => {
  const findUnique = vi.fn(async () => ({ passengerNotifiedAt: new Date() }));
  const upsert = vi.fn(async () => ({}));
...
test('notifyPassengerAboutDriverPair: sent + upsert при успішній доставці (стаб)', async () => {
  ...
  const upsert = vi.fn(async () => ({}));
  setTelegramPrismaForTests(asPrisma({ viberMatchPairNotification: { findUnique, upsert } }));
  const sendStub = vi.fn(async () => ({ sent: true, via: 'user' as const }));
  ...
  const sendCall = sendStub.mock.calls[0] as unknown as [string, string] | undefined;
  assert.ok(String(sendCall?.[1] ?? '').includes('Пряме співпадіння'));
  assert.equal(upsert.mock.calls.length, 1);
});
...
test('notifyDriverAboutPassengerPair: skipped якщо вже driverNotifiedAt', async () => {
  const findUnique = vi.fn(async () => ({ driverNotifiedAt: new Date() }));
  const upsert = vi.fn(async () => ({}));
  setTelegramPrismaForTests(asPrisma({ viberMatchPairNotification: { findUnique, upsert } }));
  setSendMatchMessageToPersonForTests(async () => ({ sent: true, via: 'bot' }));
  const passenger = {
...
test('notifyMatchingDriversForNewPassenger: findMany водіїв + upsert для кожної пари (стаб доставки)', async () => {
  ...
  const findManyDrivers = vi.fn(async () => [driver]);
  const findUnique = vi.fn(async () => null);
  const upsert = vi.fn(async () => ({}));
  setTelegramPrismaForTests(
    asPrisma({
...
test('executeBookViberRideShare: ok + rideShareRequest.create', async () => {
  ...
  const viberFindUnique = vi.fn(async (args: { where: { id?: number } }) => {
    if (args.where.id === 5) return driverRow;
    return null;
  });
  const viberFindMany = vi.fn(async () => []);
  const viberCreate = vi.fn(async ({ data }: { data: Record<string, unknown> }) => ({
    id: 88,
    senderName: data.senderName,
    phone: data.phone,
    ...data,
  }));
  const rideCreate = vi.fn(async ({ data }: { data: Record<string, unknown> }) => ({
    id: 401,
    ...data,
  }));
  setTelegramPrismaForTests(
    asPrisma({
      ...
      person: {
        ...
        findUnique: vi.fn(async (args: { where: { id?: number; phoneNormalized?: string } }) => {
  ...
  const rideData = rideCreate.mock.calls[0]?.[0] as
    | { data: { driverListingId: number; passengerListingId: number } }
    | undefined;
  assert.equal(rideData?.data?.driverListingId, 5);
  assert.equal(rideData?.data?.passengerListingId, 88);
});
...
test('executeBookViberRideShare: без телефону в Person — помилка', async () => {
  setTelegramPrismaForTests(
    asPrisma({
      ...
      person: {
        findFirst: vi.fn(async () => ({ id: 9, phoneNormalized: null, fullName: null })),
      },
    })
  );
