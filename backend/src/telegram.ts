...
function isTelegramAdminChat(chatId: string): boolean {
...
): Promise<{ id: number; phoneNormalized: string; fullName: string | null; created: boolean }> => {
  const normalized = normalizePhone(phone);
...
export const sendViberListingNotificationToAdmin = async (listing: {
  ...
  notes: string | null;
  priceUah?: number | null;
  /** Джерело рядка в БД: telegram1 — група PoDoroguem, інакше Viber */
  source?: string | null;
}) => {
  if (!bot || !adminChatId) {
    console.log('⚠️ Telegram bot або admin chat ID не налаштовано');
  ...
  try {
    ...
    const sourceLabel =
      listing.source === 'telegram1' ? 'Telegram' : 'Viber';
    const message = `
...
export async function resolveNameByPhoneFromTelegram(phone: string): Promise<string | null> {
  ...
  return withTelegramUserSessionExclusive(() => new Promise<string | null>((resolve) => {
...
async function registerUserPhone(chatId: string, userId: string, phoneInput: string, telegramName?: string | null) {
  ...
  try {
    ...
    const finishReferralLink = async (personId: number, personWasNewToClientsDb: boolean) => {
    ...
    if (linkedPersonId) await finishReferralLink(linkedPersonId, linkedPersonWasNew);
...
const ADMIN_EXTRA_BOT_COMMANDS: { command: string; description: string }[] = [
...
const ADMIN_MENU_BUTTONS = new Set<string>([
  MENU_NAV.ROOT_ADMIN,
...
function getMainMenuKeyboard(chatId?: string): TelegramBot.ReplyKeyboardMarkup {
  const rows: string[][] = [
...
function getPoputkyMenuKeyboard(): TelegramBot.ReplyKeyboardMarkup {
  return replyKeyboard([
    [MENU_NAV.ALL_RIDES],
...
function getAdminMenuKeyboard(): TelegramBot.ReplyKeyboardMarkup {
  return replyKeyboard([
...
async function handleMenuNavigation(chatId: string, text: string): Promise<boolean> {
  if (!bot) return false;

  // Адмін-кнопки: чужим чатам — тихо ігноруємо (без підказок, що команда існує).
  if (ADMIN_MENU_BUTTONS.has(text)) {
    if (!isTelegramAdminChat(chatId)) return true;
  ...
  if (text === MENU_NAV.ROOT_PROMO) {
    await bot.sendMessage(
      chatId,
      '🎁 <b>Акції</b>\nПриведи друга або підтверди поїздку фото:',
      { parse_mode: 'HTML', reply_markup: getPromoMenuKeyboard() }
    );
    return true;
  }
...
async function runAdminAddViber(chatId: string): Promise<void> {
  if (!bot || !isTelegramAdminChat(chatId)) return;
  addViberAwaitingMap.set(chatId, Date.now());
  await bot.sendMessage(
    chatId,
    '📱 <b>Додати оголошення з Вайверу</b>\n\n' +
...
async function runAdminAddTelegram(chatId: string): Promise<void> {
  if (!bot || !isTelegramAdminChat(chatId)) return;
  const keyboard = {
    inline_keyboard: [
 ...
 */
async function runAdminCheckClients(chatId: string): Promise<void> {
  if (!bot || !isTelegramAdminChat(chatId)) return;
...
📱 <b>Меню знизу:</b>
🎫 Маршрутки · 🚗 Попутки · 🎁 Акції · 📚 Довідка${
        isTelegramAdminChat(chatId) ? ' · 🛠 Адмін' : ''
      }
...
const routeKeyboard = buildPoputkyFromKeyboard('addpassenger', points);
  ...
  bot.on('message', async (msg) => {
    ...
    if (text && bot) {
      try {
        if (await handleMenuNavigation(chatId, text)) return;
      } catch (err) {
