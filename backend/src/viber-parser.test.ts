...
test('extractListingType: "заберу" — водій, навіть коли "пасажир" описує місткість авто', () => {
  // Реальний кейс з golden-датасету (#8760): без слова "водій", але "два пасажира"
  // описує заднє сидіння, а не роль автора.
  assert.equal(
    extractListingType(
      'Вітаю!\nЗавтра 09.09.26 о 5.40 з Житомира до Києва.\nТойота Королла.\nЗаберу по дорозі. На задньому сидінні два пасажира. Багаж невеликий Ціна 250 грн',
    ),
    'driver',
  );
  assert.equal(extractListingType('Заберу по місту, є 3 місця'), 'driver');
  // Справжній пасажир з "заберіть" (наказовий, до водіїв) — не плутати з "заберу"
  assert.equal(
    extractListingType(
      '2 Пасажира на Житомир 12.08 туди і назад. Водії можливо хтось їхатиме по справах, то заберіть 0683852805',
    ),
    'passenger',
  );
});
