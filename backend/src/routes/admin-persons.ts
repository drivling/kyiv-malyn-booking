...
r.get('/admin/persons', requireAdmin, async (req, res) => {
  try {
...
r.post('/admin/persons/refresh-names', requireAdmin, async (req, res) => {
  try {
...
r.post('/admin/persons/check-usernames', requireAdmin, async (req, res) => {
  try {
    const persons = await prisma.person.findMany({
      where: {
        telegramChatId: null,
...
/** Одна персона за id. */
r.get('/admin/persons/:id', requireAdmin, async (req, res) => {
  try {
...
r.put('/admin/persons/:id', requireAdmin, async (req, res) => {
  try {
    const id = parseInt(req.params.id, 10);
    if (Number.isNaN(id)) {
      res.status(400).json({ error: 'Невірний id' });
...
r.delete('/admin/persons/:id', requireAdmin, async (req, res) => {
  try {
    const id = parseInt(req.params.id, 10);
    if (Number.isNaN(id)) {
      res.status(400).json({ error: 'Невірний id' });
