...
- Dzhura (chat capture, phase 0): `backend/src/dzhura.test.ts` (Kyiv day range → UTC incl. DST, date validation, export payload with BigInt → string), `backend/src/admin-dzhura-http.test.ts` (auth, lunch-group lock, jobs validation/409, export headers), `frontend/.../DzhuraTab.test.tsx` (flags PATCH, backfill polling, JSON download via Blob, listener status).
